...
def follow_acc(list):
    for i in range(len(list)):
        if i >= len(list):
            print('done')
            break
        # get user_id to be used
        user_info = api.username_info(list[i])
        user_id = user_info['user']['pk']
        # print back username with its user_id
        print(list[i] + ' - ' + str(user_id))
        follow = api.friendships_create(user_id)
        # print response
        print(follow)
        # random minutes for the next user
        random = randrange(4,6)
        print('waiting for ' + str(random) + ' minutes...')
